...
function AppContent() {
  ...
  //local state use for Snackbar visibility
  ...
  const handleUpdate = useCallback(
    (id: string, patch: Partial<Task>) => {
      updateTask(id, patch);
      setActivity((prev) =>
        [createActivity('update', `Updated: ${Object.keys(patch).join(', ')}`), ...prev].slice(
          0,
          50,
        ),
      );
  ...
  const handleDelete = useCallback(
    (id: string) => {
      ...
      // for current delete to show snackback
      setShowUndo(true);
    },
